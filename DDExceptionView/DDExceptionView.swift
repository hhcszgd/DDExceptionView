...
class DDExceptionView: DDMaskBaseView {
    ...
    var exception : DDExceptionModel = DDExceptionModel(title: "there is something wrong", image: "login_applogo"){
        didSet{
            layoutIfNeeded()
            setNeedsLayout()
        }
    ...
    override func layoutSubviews() {
        ...
        self.errorMessage.frame = CGRect(x: 0, y: self.bounds.height/2, width: self.bounds.width, height: 34)
